          ...
          {users.map((u) => {
            ...
            return (
              <button key={u.id} className={`ad2-user ${selected === u.id ? "on" : ""}`}
                ...
                {u.invited && !u.password_hash
                  ? <span className="ad2-off ad2-off--pending">invited</span>
    ...
    <>
      ...
      <div className="ad2-row">
        ...
        <label className="ad2-f"><span>Role</span>
          <select className="ad2-in" value={f.role_code}
      ...
      <div className="ad2-rolepreview">
        ...
        <div className="ad2-previewlist">
          {(ROLE_DEFAULTS[f.role_code] ?? []).map((c) => (
      ...
      <div className="ad2-note">
        The account is created inactive with no password, and an invitation goes to
        that address. They choose their own password from the link, which works once
        and expires in 72 hours.
        <br /><br />
        Nobody is sent a password, including by us. A password in an email sits in
        two mailboxes forever, and it is the one credential the sender can also read.
      </div>

      <div className="ad2-actions">
        <button className="ad2-btn" disabled={!ok || busy}
                onClick={async () => {
                  setBusy(true); setErr("");
                  try {
                    ...
                    onAdd({ ...d.user, ...f, is_active: false, invited: true,
                            created_at: nowISO() });
                  } catch {
          ...
          {busy ? "Sending the invitation…" : "Create and invite"}
        </button>
        <button className="ad2-btn ad2-btn--ghost" onClick={onCancel}>Cancel</button>
      </div>
      {err && <div className="ad2-err">{err}</div>}
    </>
  );
}
...
.ad2-off--pending{color:#C98A15;border-color:#E8C877}
...
.ad2-err{font-size:12.5px;color:var(--red);background:#FDF6F7;border:1px solid var(--red);
