...
| `src/routes/auth.js` | Sign in, reset, session |
...
The rest of `server/src/routes/`. Order:

```
1. core          units, pricing, parking
2. accounting    the largest, and the one that most needs real transactions
3. workflow      documents, move-out, maintenance
4. the rest
```
...
Keep `server/` until this is finished. During the port it is the only correct
