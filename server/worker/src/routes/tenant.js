...
r.get("/public/slots", async (c) => {
  const sql = c.get("db");
  const unitType = c.req.query("unit_type");
  ...
  for (let i = 0; i < 14; i++) {
    ...
    for (let h = OPEN; h < (dow === 6 ? SAT_CLOSE : CLOSE); h++) {
      for (const m of [0, SLOT]) {
        const at = new Date(`${iso}T${String(h).padStart(2, "0")}:${String(m).padStart(2, "0")}:00`);
  ...
  return c.json({ days, occupied, notice_hours: noticeHours,
    note: occupied
...
r.post("/public/tenant/login", async (c) => {
  ...
  let ok = false;
  try {
    ok = !!(a?.is_active && a.password_hash && await verifyPassword(password, a));
  } catch (e) {
  ...
  if (!ok) {
    if (a) {
      ...
      await sql`UPDATE tenant_accounts SET failed_attempts = ${n},
        locked_until = ${n >= LOCK_AFTER
...
r.get("/tenant/viewings", async (c) => {
  ...
  return c.json({ viewings: await c.get("db")`
    SELECT sr.id, sr.unit_type, sr.preferred_at, sr.state, sr.created_at,
    ...
    ORDER BY COALESCE(e.starts_at, sr.preferred_at) DESC NULLS LAST
    LIMIT 50` });
});
...
r.get("/tenant/applications", async (c) => {
  ...
  const rows = await c.get("db")`
    SELECT id, unit_type, state, move_in_date, occupants, created_at, decided_at
  ...
  return c.json({ applications: rows.map((a) => ({ ...a,
    ...
    // the person waiting.
    plain: a.state === "submitted" ? "With us. We will be in touch."
      : a.state === "screening" ? "Being reviewed."
...
r.get("/tenant/repairs", async (c) => {
  ...
  return c.json({ repairs: await c.get("db")`
    SELECT tr.id, tr.what, tr.where_in_unit, tr.urgent, tr.created_at,
           m.state AS ticket_state, m.scheduled_at, m.vendor
  ...
  if (a) {
    ...
    await sql.begin(async (tx) => {
      await tx`INSERT INTO email_verifications (id, purpose, email, unit_number,
        ...
        VALUES (${uid("ev_")}, 'tenant_claim', ${a.email.toLowerCase()},
  ...
  try {
    await sql.begin(async (tx) => {
      ...
      await tx`UPDATE tenant_accounts SET password_algo = ${h.algo},
        ...
        WHERE unit_number = ${v.unit_number} AND is_active`;
