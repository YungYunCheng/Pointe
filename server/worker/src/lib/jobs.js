...
async function scanRenewals(sql, today) {
  ...
  for (const l of ending)
    await sql`INSERT INTO notifications (id, role_code, kind, code, payload, link)
...
async function applyDueIncreases(sql, today) {
  const due = await sql`
    SELECT ri.*, l.status AS lease_status, l.rent AS lease_rent
    FROM rent_increases ri
    JOIN leases l ON l.id = ri.lease_id
    WHERE ri.state = 'served' AND ri.effective_on <= ${today}`;
  ...
  for (const ri of due) {
    if (ri.lease_status !== "active") {
      skipped.push({ unit: ri.unit_number, why: "tenancy ended" });
      await sql`UPDATE rent_increases SET state = 'void' WHERE id = ${ri.id}`;
      continue;
    }
    ...
    await sql.begin(async (tx) => {
      await tx`UPDATE leases
        SET rent = ${ri.new_rent}, last_increase_at = ${ri.effective_on}
        WHERE id = ${ri.lease_id}`;

      // Rent only. Parking and storage are separate agreements with their own
      // figures, and moving them because the rent moved would be an increase
      // nobody was given notice of.
      await tx`UPDATE charge_schedules SET amount = ${ri.new_rent}
        WHERE lease_id = ${ri.lease_id} AND kind = 'rent' AND is_active`;

      await tx`UPDATE rent_increases SET state = 'applied', applied_at = now()
        WHERE id = ${ri.id}`;

      await tx`INSERT INTO rent_increase_events (id, increase_id, event, detail)
        VALUES (${uid("rie_")}, ${ri.id}, 'applied',
                ${`${ri.current_rent} to ${ri.new_rent} from ${ri.effective_on}`})`;

      await tx`INSERT INTO notifications (id, role_code, kind, code, payload, link)
        VALUES (${uid("nt_")}, 'accounting', 'accounting', 'RENT_INCREASE_APPLIED',
                ${JSON.stringify({ unit: ri.unit_number, from: ri.current_rent,
                                   to: ri.new_rent, effective: ri.effective_on })},
                '/accounting')`;
    });
    applied++;
  }

  if (applied) console.log(`[increases] ${applied} applied`);
  if (skipped.length) console.warn(`[increases] ${skipped.length} skipped`, skipped);
