...
function SignIn({ onIn }) {
  ...
  return (
    <section className="bt-sec"><div className="bt-form">
      ...
      {/* First time here. Not "sign up" — the account is claimed against a
          lease that already exists, and calling it signing up would suggest
          anybody can make one. */}
      <div className="bt-firsttime">
        <strong>{t("portal.firstTimeHeading")}</strong>
        <p>{t("portal.firstTimeBody")}</p>
        <a className="bt-btn bt-btn--ghost" href="/signup">{t("portal.setUp")}</a>
      </div>
    </div></section>
  );
}
