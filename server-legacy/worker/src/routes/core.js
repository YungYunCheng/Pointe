...
r.post("/pricing", require_("settings.pricing.edit"), async (c) => {
  const sql = c.get("db");
  const { effective_from, rents, fees, note } = await c.req.json();
  ...
  const out = await sql.begin(async (tx) => {
    ...
    const [profile] = await tx`
      INSERT INTO pricing_profiles (id, effective_from, note, created_by)
      VALUES (${uid('pp_')}, ${effective_from},
              ${note ?? null}, ${c.get("user").id})
      RETURNING *`;
