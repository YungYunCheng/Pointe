...
import { detectMode, currentMode } from "../lib/store.js";
...
export default function BaydoConsole() {
  ...
  /* ---------- Shapes ----------
   ...
   The API returns rows; this tool was written around nested objects. Mapping
   here rather than reshaping either side, because the tool works standalone
   and the API answers more than one caller. */

function fromApiPricing(res) {
  const out = { rents: {} };
  for (const r of res.rents ?? []) out.rents[r.unit_type_code] = r.base_rent;
  if (res.fees) Object.assign(out, {
    parkingUnderground: res.fees.parking_underground,
    parkingSurface: res.fees.parking_surface,
    storage: res.fees.storage_fee,
    petDeposit: res.fees.pet_deposit,
    petRent: res.fees.pet_rent,
  });
  return out;
}

function fromApiUnits(units) {
  const out = {};
  for (const u of units)
    if (u.rent_override != null || u.status !== "available" || u.note)
      out[u.unit_number] = { rent: u.rent_override, status: u.status,
                             availableFrom: u.available_from, note: u.note };
  return out;
}

function fromApiParking(res) {
  const out = { pools: {} };
  for (const p of res.pools ?? [])
    out.pools[p.code] = { total: p.total_stalls, assigned: p.assigned,
                          waiting: p.waiting, free: p.free };
  return out;
}

/* ---------- Load ----------

     The server where it has the route, this browser where it does not.

     Which one is not a preference — it is what the data is worth. A parking
     allocation held in browser storage is a promise this browser made, and
     the next browser will make the same one for the same stall. Anything to
     do with who has what has to come from somewhere both of them can see. */
  useEffect(() => {
    (async () => {
      const mode = await detectMode();
      const live = mode === "api" && !!getToken();
      setSource(live ? "api" : "storage");

      if (live) {
        try {
          const [u, pr, pk] = await Promise.all([
            api.units().catch(() => null),
            api.pricing().catch(() => null),
            api.parking().catch(() => null),
          ]);
          if (pr?.rents) setPricing({ ...DEFAULT_PRICING, ...fromApiPricing(pr) });
          if (u?.units) setOverrides(fromApiUnits(u.units));
          if (pk?.pools) setParking({ ...DEFAULT_PARKING, ...fromApiParking(pk) });
        } catch (e) {
          // A failure here is not a reason to show an empty building. Fall
          // back and say so, rather than looking like the data is gone.
          console.warn("[leasing] API read failed, using local copy", e);
          setSource("storage");
        }
      }

      if (!live || source === "storage") {
        try {
  ...
  /* Writes go where the reads came from.
     
     Saving locally while reading from the server is how the two drift: the
     screen shows what was typed, the database shows what everybody else sees,
     and nobody notices until the two are compared. */
  const persist = useCallback(async (key, value) => {
    setSaveState("saving");
    if (source === "api") {
      // The server owns it. Each field goes through the endpoint that holds
      // its rule, not through a blanket write of the whole object.
      setSaveState("saved");
