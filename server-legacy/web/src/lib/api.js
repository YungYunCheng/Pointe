...
/* Empty by default, which means /api on whatever host served the page.
   The same Worker answers on both hostnames, so a request from the staff
   site to /api is same-origin — no preflight, no SameSite problem, and no
   allowed-origins list to keep in step with reality.

   VITE_API_URL is only for pointing a local front end at a deployed API. */
