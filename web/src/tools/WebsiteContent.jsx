...
const SLOTS = [
  ["hero", "Hero photo", "Large first image on the home page"],
  ["amenities", "Amenities photo", "Gym, lounge, pet wash or another shared space"],
  ["neighbourhood", "Neighbourhood photo", "Exterior, LRT or the surrounding area"],
];
...
export default function WebsiteContent() {
  ...
  const upload = async (slot, fileList) => {
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        await api.uploadWebsiteImage(file, slot, file.name.replace(/\.[^.]+$/, ""), "", i);
      }
  ...
  return <main className="wc"><style>{CSS}</style>
    ...
    <section className="wc-card"><div className="wc-cardhead"><div><h2>Footer contact</h2><p>This changes only the public website.</p></div></div>
    ...
    <section className="wc-card"><div className="wc-cardhead"><div><h2>Page photos</h2><p>JPG, PNG, WebP or AVIF · maximum 10 MB · landscape works best.</p></div></div>
      <div className="wc-slots">{SLOTS.map(([slot, label, hint]) => { const image = images(slot)[0]; return <article key={slot}>
        <div className="wc-preview">{image ? <img src={image.url} alt="" /> : <span>No photo yet</span>}</div><div className="wc-slotcopy"><strong>{label}</strong><p>{hint}</p>
          <div className="wc-actions"><Upload label={busy === slot ? "Uploading…" : image ? "Replace" : "Upload"} disabled={!!busy} onFiles={(files) => upload(slot, files)} />
            {image && <button className="wc-btn ghost" disabled={!!busy} onClick={() => remove(image)}>Remove</button>}</div></div></article>; })}</div>
