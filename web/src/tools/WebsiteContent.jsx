...
export default function WebsiteContent() {
  ...
  return <main className="wc"><style>{CSS}</style>
    ...
    <section className="wc-card"><div className="wc-cardhead"><div><h2>Page photos</h2><p>Each section accepts several photos. JPG, PNG, WebP or AVIF · maximum 10 MB each.</p></div></div>
      ...
      <div className="wc-gallery wc-amenities-gallery"><div className="wc-cardhead"><div><h2>Amenities Gallery · Buildings page</h2><p>Upload facility photos such as the gym, lounge, games room, pet wash, bike storage and outdoor patio. Four photos appear at a time between the building cards and amenities list.</p></div><Upload multiple label={busy === "amenities" ? "Uploading…" : "Add amenity photos"} disabled={!!busy} onFiles={(files) => upload("amenities", files)} /></div>
        <div className="wc-grid">{images("amenities").length ? images("amenities").map((image) => <figure key={image.id}><img src={image.url} alt="" /><button aria-label="Remove" disabled={!!busy} onClick={() => remove(image)}>×</button><figcaption>{image.filename}</figcaption></figure>) : <div className="wc-empty">No amenity photos yet. Upload at least four for the full desktop layout.</div>}</div>
      </div>
