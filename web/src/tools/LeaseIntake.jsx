...
/* ============================================================
   ...
   The line: the AI fills variables only. It never writes or edits clause text.
...
/* ---------- Approved clause library. The AI may include or omit, never rewrite. ---------- */
const CLAUSES = [
  { id: "C-001", title: "Premises and term",                always: true },
  { id: "C-002", title: "Rent and payment",                 always: true },
  { id: "C-003", title: "Security deposit and trust account", always: true },
  { id: "C-004", title: "Move-in and move-out inspection reports", always: true },
  { id: "C-005", title: "Repairs and notice of entry",      always: true },
  { id: "C-006", title: "Termination and notice periods",   always: true },
  { id: "C-007", title: "Parking addendum",   when: (v) => v.parking && v.parking !== "None" },
  { id: "C-008", title: "Storage addendum",   when: (v) => v.storage === "Yes" },
  { id: "C-009", title: "Pet addendum",       when: (v) => v.pets && v.pets !== "None" },
  { id: "C-010", title: "End of a fixed term", when: (v) => v.term && v.term.startsWith("Fixed") },
  { id: "C-011", title: "Ending a month-to-month tenancy", when: (v) => v.term === "Month to month" },
];
...
export default function ContractConsole() {
  ...
  const clauses = CLAUSES.filter((c) => c.always || (c.when && c.when(merged)));
...
{"extracted":{"field":"value"},"next_question":"the next question, or null","done":false}`;
  ...
  const doSign = () => {
    setSigned({
      ...
      clauses_included: clauses.map((c) => c.id),
      clause_library_version: "2026-07-01 (approved)",
  ...
  return (
    <div className="ct">
      ...
      <div className="ct-rule">
        <strong>Where the line sits</strong>: the AI fills variables and never writes or edits clause text. The clauses below come from the approved library,
        and the system only decides which of them apply. No clause is generated by a model.
      </div>
      ...
      <div className="ct-grid">
        ...
        <div className="ct-side">
          ...
          <section className="ct-card">
            <h2>Clauses included <span className="ct-n">{clauses.length}</span></h2>
            <p className="ct-note">
              The clause text comes from the approved library, version 2026-07-01. The system only decides which clauses apply, based on the variables.
              No clause is generated, edited or rewritten by a model.
            </p>
            <div className="ct-clauses">
              {CLAUSES.map((c) => {
                const on = c.always || (c.when && c.when(merged));
                return (
                  <div className={`ct-cl ${on ? "on" : ""}`} key={c.id}>
                    <span className="ct-mono ct-cl-id">{c.id}</span>
                    <span>{c.title}</span>
                    <span className="ct-cl-s">{c.always ? "Always" : on ? "Included" : "Not included"}</span>
                  </div>
                );
              })}
            </div>
      ...
      <footer className="ct-foot">
        Prototype. The lease template, the clause library and the checks here — the deposit cap, the inspection reports, the notice periods —
...
.ct-steps button.on{background:var(--ink);color:#fff}
...
.ct-btn{font:inherit;font-weight:600;font-size:13px;cursor:pointer;background:var(--ink);color:#fff;
