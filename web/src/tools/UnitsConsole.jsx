  ...
  return <section className="uc uc-profile">
    ...
    <header className="uc-profile-head">
      ...
      <div className="uc-profile-status"><i style={{ background: STATUS[unit.status]?.color }} />
        ...
        <span>{unit.resident?.full_name || "No active resident"}</span></div>
    </header>
    ...
    {tab === "ledger" && <UnitLedger unit={unit} session={session} />}
...
function UnitLedger({ unit, session }) {
  ...
  return <section className="uc-ledger">
    ...
    {data && <>
      <div className="uc-ledger-stats">
        <Stat label="Current balance" value={money2(data.summary.balance)} />
      ...
      {canEdit && mode && <LedgerEntryForm mode={mode} unit={unit} methods={methods}
        onCancel={() => setMode(null)} onSaved={async () => { setMode(null); await load(); }} />}
...
function LedgerEntryForm({ mode, unit, methods, onCancel, onSaved }) {
  ...
  const [form, setForm] = useState(mode === "payment" ? {
    ...
    received_on: today, reference: "", note: "",
  ...
  const save = async () => {
    ...
    setBusy(true); setError("");
    try {
      if (mode === "payment") await api.manualPayment({ ...form, unit_number: unit.unit_number });
      else await api.addLedgerCharge(unit.unit_number, form);
      await onSaved();
  ...
  return <section className="uc-entry-form"><h3>{mode === "payment" ? "Record a payment received" : "Add a ledger charge or credit"}</h3>
    <div className="uc-form-grid">{mode === "payment" ? <>
      <label><span>Payment method</span><select value={form.method_code} onChange={(e) => set({ method_code: e.target.value })}>
      ...
      <label><span>Reference / cheque no.</span><input value={form.reference} onChange={(e) => set({ reference: e.target.value })} /></label>
    ...
    {error && <div className="uc-error">{error}</div>}
    <div className="uc-form-actions"><button className="uc-primary" disabled={busy || !(Number(form.amount) > 0) || (mode === "charge" && !form.description.trim())} onClick={save}>{busy ? "Saving…" : "Post entry"}</button>
