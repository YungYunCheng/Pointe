...
r.get("/renewals", require_("units.view"), async (c) => {
      ...
      AND (
        ...
        (l.end_date IS NULL
         AND (l.last_increase_at IS NULL
              OR l.last_increase_at <= (CURRENT_DATE - INTERVAL '1 day' * ${INCREASE_INTERVAL_DAYS}))
         AND NOT EXISTS (
  ...
  return c.json({
    ...
    rules: { increase_interval_days: INCREASE_INTERVAL_DAYS, notice_days: END_NOTICE_DAYS,
...
async function applyRenewal(tx, o) {
  ...
  const fixedTerm = Number(o.term_months) === 6 ? "fixed_6"
    : Number(o.term_months) === 12 ? "fixed_12" : "fixed";
  ...
  await tx`UPDATE leases
    SET term_type = ${periodic ? "periodic" : fixedTerm},
