...
import { INCREASE_INTERVAL_DAYS, INCREASE_NOTICE_DAYS,
  MONTHLY_INCREASE_NOTICE_MONTHS, DEEMED_SERVICE_DAYS,
  NEEDS_PROOF_OF_SERVICE } from "../lib/rules.js";
...
/* Three full tenancy months is not 90 days. Start at the next tenancy-period
   boundary, then count three complete periods. Lease commencement supplies
   the boundary day; 28 is the safe ceiling for every month. */
function addFullTenancyMonths(servedOn, commencement, count = MONTHLY_INCREASE_NOTICE_MONTHS) {
  const served = new Date(`${servedOn}T12:00:00Z`);
  const day = Math.min(Number(String(commencement ?? "").slice(8, 10)) || 1, 28);
  let boundary = new Date(Date.UTC(served.getUTCFullYear(), served.getUTCMonth(), day, 12));
  if (boundary <= served) boundary = new Date(Date.UTC(
    served.getUTCFullYear(), served.getUTCMonth() + 1, day, 12));
  boundary.setUTCMonth(boundary.getUTCMonth() + count);
  ...
  for (const leaseId of lease_ids) {
    try {
      const one = await sql.begin(async (tx) => {
        ...
        const earliestByNotice = addFullTenancyMonths(today(), lease.start_date);
...
r.post("/increases/:id/serve", require_("settings.pricing.edit"), async (c) => {
  const sql = c.get("db");
  const { method = "email", served_on, witness, message } = await c.req.json();

  try {
    const out = await sql.begin(async (tx) => {
      const [ri] = await tx`SELECT ri.*, l.start_date AS lease_start_date
        FROM rent_increases ri JOIN leases l ON l.id = ri.lease_id
        WHERE ri.id = ${c.req.param("id")} FOR UPDATE OF ri`;
      ...
      if (method === "email" && (!ct?.electronic_service_email ||
          ct.electronic_service_email.toLowerCase() !== String(ct.email ?? "").toLowerCase()))
        throw Object.assign(new Error("EMAIL_NOT_ADDRESS_FOR_SERVICE"), { status: 409,
          detail: "Record this tenant's email as an electronic address for service before serving a legal notice by email." });
      ...
      const earliest = addFullTenancyMonths(deemed, ri.lease_start_date);
