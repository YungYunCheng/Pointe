...
import { TenantAuthProvider, useTenantAuth, GatedLink, RequireAuth } from "./lib/auth.jsx";
  ...
  return (
    <header className="bt-head">
      ...
      {/* Two halves.
          ...
          Suites and the buildings are how somebody decides whether this is
          worth a visit, so they stay open. Applying and booking produce a
          decision and a slot in a diary, so they appear once there is an
          account — and they appear rather than sitting there greyed out,
          because a disabled control invites a click that goes nowhere.
          
          The signed-out call to action is the account itself, phrased as the
      ...
      <nav className={`bt-nav ${open ? "open" : ""}`} onClick={() => setOpen(false)}>
        ...
        {session && <NavLink to="/apply" className={link}>{t("nav.apply")}</NavLink>}
        <NavLink to="/portal" className={link}>
          {session ? t("nav.portal") : t("nav.signin")}
        </NavLink>
        ...
        {session
          ...
          : <Link to="/signup?next=%2Fbook" className="bt-cta">{t("nav.getStarted")}</Link>}
      </nav>
    </header>
  );
}
    ...
    <div className="bt">
      ...
      <main>
        <Routes>
          ...
          {/* Checked here as well as hidden in the navigation. Somebody with
              the URL, or a bookmark from last week, arrives regardless of
